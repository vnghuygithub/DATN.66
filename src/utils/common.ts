...
export const convertVietnameseToEnglish = (text: string) => {
  ...
  return text
    .split('')
    .map((char: string) => {
      const indexLowercase = vietnameseLettersLowercase.indexOf(char);
      const indexUppercase = vietnameseLettersUppercase.indexOf(char);

      if (indexLowercase !== -1) {
        return englishLettersLowercase.charAt(indexLowercase);
      } else if (indexUppercase !== -1) {
        return englishLettersUppercase.charAt(indexUppercase);
      }
  ...
  const filteredObj = obj.filter(department => {
    ...
    const filteredChildren = department.children.filter((employee: any) => {
      ...
      const normalizedEmployeeCode = normalizeString(
        employee.time_keeping_code || ''
      );
      const normalizedSearchName = normalizeString(employeeName || '');
      const normalizedSearchCode = normalizeString(time_keeping_code || '');

     
      const isNameMatched =
        !employeeName || normalizedEmployeeName.includes(normalizedSearchName);
      const isCodeMatched =
        !time_keeping_code || normalizedEmployeeCode.includes(normalizedSearchCode);
...
export function transformObject(oldObject: any) {
  ...
  oldObject.forEach((item: any) => {
    ...
    const dateObj: any = {
      ...
      probation_wage_rate: item.probation_wage_rate,
      probation_completion_wage: item.probation_completion_wage,
      minutes_working_reduced: item.minutes_working_reduced,
    };
  ...
  // console.log('MAObject', newObject);
  return newObject;
}
