...
export const getAttendanceReportSelf = async (data: IGetAttendanceParams) => {
  const res = await request('post', GET_ATTENDANCE.SEARCHSELF, data);
  if (res?.result?.error?.code && res.result.error.code == 400) {
    $message.error(res.result.error.message);
    return;
  }
...
export const getListShifts = async (company_id: number) => {


  const res = await request('get', SHIFTS.GETALL+`?filter=[["company_id","=",${company_id}]]`)
  if (res?.result?.error?.code && res.result.error.code == 400) {
    $message.error(res.result.error.message);
    return;
  }
  ...
  keys.forEach(key => {
    if (!isNaN(Number(key))) {
      ...
      if (item.valueSearch.length === 2) {
        ...
        transformedData2.push(['request_date_from', '>=', startDate]);
        transformedData2.push(['request_date_to', '<=', endDate]);
      } else {
...
export const getLeaveListApprovalOnly = async (
  args: ILeaveListApprovalArgs
) => {
  const keys = Object.keys(args);
  if (args.dateRange && args.dateRange.length === 2) {
    args.start_date = moment(args.dateRange[0]).format('YYYY-MM-DD');
...
export const getLeaveListApprovalHrOnly = async (
  args: ILeaveListApprovalArgs
) => {
  let url = GET_LIST_LEAVE.GETLEAVEAPPROVALONLY;
  let requestBody = {
