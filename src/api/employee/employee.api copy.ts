...
import { DOMAIN_AUDIT_LOG_HISTORY, EMPLOYEELIST, SHIFTSLIST } from '../constApi';
import { request } from '../request';
import { mapEmployee, mapEmployeeDtoToEmployeee, mapEmployeeLog } from './transform';
...
export const getEmployeeParent = async (id: any) => {
  let url = EMPLOYEELIST.PARENT + `&filter=[["id","=",${id}],["company_id","=",${Number(localStorage.getItem('company_id'))}]]`;
  try {
    const res = await request('get', url);
    if (res?.result?.error?.code && res.result.error.code == 400) {
      $message.error(res.result.error.message);
...
export const getEmployeeParentNoArgs = async (employeeId: any) => {
  ...
  if (employeeParent) {
    let employeeParentId = employeeParent?.result?.filter((item: any) => item.id == employeeId)?.parent_id
    let url = EMPLOYEELIST.PARENT
    if (employeeParentId) {
...
export const getEmployeeParentNoArgsV2 = async () => {
  let url = EMPLOYEELIST.PARENT + `&filter=[["company_id","=",${Number(localStorage.getItem('company_id'))}]]`
  try {
    const res = await request('get', url);
    if (res?.result?.error?.code && res.result.error.code == 400) {
      $message.error(res.result.error.message);
...
export const getListEmployeeV2 = async (id?: number) => {
...
export const getEmployeeByArgs = async (args: IFilterEmployeesArgs) => {
  ...
  let requestBody = {
    params: {
      args: [
        args.name ? convertVietnameseToEnglish(args.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")).toLowerCase().trim() : '',
        (args.code ?? '').trim(),
...
export const getEmployeeRelativeByEmployeeId = async (id: number) => {
...
export const getEmployeeRelativeByEmployeeIds = async (id: number) => {
  let url = `api/hr.employee.relative.register/?filter=[["apec_register_employee_id","=",${id}]]`;
  console.log(id)
  const res = await request('get', url);
  console.log("7777",res.result)
  if (res?.result?.error?.code && res.result.error.code == 400) {
    $message.error(res.result.error.message);
    return;
  }
