...
const SelectDepartmentEmployeeForm = ({ requiredOption ,isCreating,selectedCompanyId,onChange,disabled, ...props }: any) => {
    const [department, setDepartment] = useState<IDepartment[]>([]);
    const [isMounted, setIsMounted] = useState(true);
    const is_administrative = localStorage.getItem('is_administrative');
    useEffect(() => {
        setIsMounted(true);
        departmentOptions("",selectedCompanyId).then((res) => {
            if (isMounted)
                setDepartment(res);
