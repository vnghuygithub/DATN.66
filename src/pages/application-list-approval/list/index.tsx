...
import SearchApplicationListApprovalMis from '../components/search-mis';

const ApplicationListApprovalOnly = () => {
  const { t } = useLocale();
  ...
  const searhListLeave = async (args: ILeaveListApprovalArgs) => {
    const res = await getLeaveListApprovalOnly(args);
    if (res) {
      return res;
    }
  };
    ...
    <>
     <Modal
      ...
      <MyPage
        ...
        title={'Danh sách đơn yêu cầu đã duyệt cấp 1'}
        searchRender={<SearchApplicationListApprovalMis />}
...
export default ApplicationListApprovalOnly;
