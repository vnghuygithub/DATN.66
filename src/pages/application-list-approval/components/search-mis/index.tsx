...
const SearchUserMis = () => {
  ...
  const handleFieldChange3 = (value: any, id: any) => {
    const newSelects = rows.map(select => {
      if (select.id === id) {
        if (value.name === 'employee_name_unaccent') {
          const valueConvert = convertVietnameseToEnglish(
            ...
            .toLowerCase()
            .trim();
          return { ...select, valueSearch: valueConvert };
        } else {
  ...
  return (
    <div style={{ display: 'flex', alignContent: 'center' }}  onKeyDown={e => {
      if (e.key === 'Enter') {
        e.preventDefault();
        handleSubmitForm();
      }
      ...
      <Row
        style={{ display: 'flex', flexWrap: 'wrap', alignContent: 'center' ,flexDirection:"column"}}>
        {rows.map(row => (
          <div
            key={row.id}
            style={{
