...
import {
  ILeaveListApprovalArgs,
  IUpdateSummaryArgs,
  calculateAttendanceReport,
  deleteLeave,
  getLeaveListApprovalHrOnly,
  getLeaveListApprovalOnly,
  searchListLeave,
  updateSummaryReport,
...
import ApplicationFormApproval from '../handle';
import SearchApplicationListApproval from '../components/search';
import ApplicationFormApprovalHr from '../handle';
import SearchApplicationListApprovalHr from '../components/search';
import SearchApplicationListMis from '@/pages/application-list/components/search-mis';
import { render } from 'react-dom';

const ApplicationListApprovalOnlyHr = () => {
  const { t } = useLocale();
  ...
  const tableColums: MyPageTableOptions<IApplicationList> = [
    ...
    // },
    {
      ...
      render: (item: string) => {
        ...
        } else if (item.toLowerCase() === 'validate1') {
          return (
            <Tag className="tags" color="#FF6600">
              Đã duyệt cấp 1
            </Tag>
          );
        }
    ...
    },
    {
      ...
      align: 'left',
      render: item => {
        return <div>{item === '1' ? 'Cá nhân' : 'Công việc'}</div>;
      },
    },
    {
      title: 'Ngày tạo',
  ...
  const searhListLeave = async (args: ILeaveListApprovalArgs) => {
    const res = await getLeaveListApprovalHrOnly(args);
    if (res) {
      console.log(res);
      return res;
    }
  ...
  const _updateStatusLeaveList = async (selectedRow: any[], status: string) => {
    setStatus((status === "validate" || status === "validate1") ? 'duyệt': 'hủy');
  ...
  return (
    ...
    <Modal
      ...
      <MyPage
        ...
        title={'Danh sách đơn yêu cầu duyệt cấp 2'}
...
export default ApplicationListApprovalOnlyHr;
