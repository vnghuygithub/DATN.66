...
import { IListLeavereq } from '@/interface/weeklyreport-test/type';
...
const index = (props: ILeaveFormProps) => {
  ...
  const getListLeaveType = () => {
    ...
    getListLeave(body).then(res => {
      ...
      console.log('dataLeaveType', res.result);
  ...
  useEffect(() => {
    if (detailAttendanceSelected?.holiday_status_id.type === 'TC') {
      ...
      console.log(typeLeaveValue);
      console.log(detailAttendanceSelected?.holiday_status_id);
      if (
        typeLeaveValue !==
      ...
      if (
        detailAttendanceSelected?.holiday_status_id.type === 'DXVS' ||
        detailAttendanceSelected?.holiday_status_id.type === 'DXDM'
      ) {
        setTypeLeaveValue(
          detailAttendanceSelected?.holiday_status_id.type +
            '~' +
            detailAttendanceSelected?.holiday_status_id.id
        );
      } else {
        setTypeLeaveValue(
          detailAttendanceSelected?.holiday_status_id.type +
  ...
  useEffect(() => {
    if (
      ...
      if (typeLeaveInfo[0] && typeLeaveInfo[0] === 'TC') {
        ...
        (typeLeaveInfo[0] === 'P' ||
          typeLeaveInfo[0] === 'B' ||
          typeLeaveInfo[0] === 'DXVS' ||
          typeLeaveInfo[0] === 'DXDM')
      ) {
        form.setFieldsValue({
          leaveDay1: [
  ...
  useEffect(() => {
    if (
      ...
      const defaultDates = [moment(date), moment(date)];
      form.setFieldsValue({ leaveDay1: defaultDates });
    }
  }, [typeLeaveValue, date]);
  ...
  useEffect(() => {
    if (typeLeaveInfo[0] !== undefined && typeLeaveInfo[0] === 'DXDM') {
      form.setFieldsValue({
        restMinutes: attendance_late ? attendance_late : '',
      });
    } else {
      form.setFieldsValue({
        restMinutes: leave_early ? leave_early : '',
      });
    }
  }, [typeLeaveInfo[0], attendance_late, leave_early]);
  const items = [
    {
      key: '1',
  ...
  return (
    ...
    <Row gutter={24}>
      ...
      <Col span={12}>
        ...
        {(typeLeaveInfo[0] === 'DXVS' || typeLeaveInfo[0] === 'DXDM') && (
          <Col
            style={{
              marginTop: '25px',
            }}>
                ...
                <Form.Item
                  label={
                    typeLeaveInfo[0] === 'CT'
                      ...
                      : 'Số phút nghỉ và thời gian nghỉ'
                  }
                  name="restMinutes"
                  required>
                  <Input
        ...
        <Row gutter={24}>
          <Col span={24}>
            <Form.Item
              ...
              <TextArea
                rows={2}
                placeholder="Lý do xin nghỉ tối đa 255 ký tự"
                maxLength={255}
                allowClear
              />
            </Form.Item>
