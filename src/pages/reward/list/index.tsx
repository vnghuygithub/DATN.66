...
import {
  deleteJobTypeFromReward,
  getRewardContents,
} from '@/api/reward/reward.api';
...
const RewardContentList: FC = () => {
  ...
  const [openRewardContentGeneral, setOpenRewardContentGeneral] =
    useState(false);
  ...
  const handleDelete = async () => {
    ...
    await Promise.all(
      objectId.map(item => deleteJobTypeFromReward(item.id, item.job_type_id))
    )
      .then(res => {
        if (res) {
