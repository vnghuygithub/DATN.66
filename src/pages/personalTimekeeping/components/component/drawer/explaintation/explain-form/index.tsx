...
const index = (props: ILeaveFormProps) => {
    ...
    if (isCreating) {
      ...
      if (data.invalid_type === 5) {
        ...
        if (
          ...
          $message.error('Vui lòng chọn giờ  bắt đầu ra ngoài');
          store.dispatch(
            setGlobalState({
              loading: false,
            })
  ...
  useEffect(() => {
    if (isView) {
      ...
      setInvalidType(Number(detailAttendanceSelected.invalid_type));
      form.setFieldsValue({
        reason: Number(detailAttendanceSelected.reason) || 1,
        remarks: detailAttendanceSelected.remarks
          ? detailAttendanceSelected.remarks
          : '',
        keep_in_time: detailAttendanceSelected.keep_in_time
  ...
  console.log(attendance_attempt_1);
  //console.log(attendanceMissingFrom)
  const items2 = [
    {
      key: '1',
  ...
  return (
    <BaseForm
      ...
      {(setNonNullUndefinedattendance_inoutArr).length >0 ? (
         <Row gutter={24}>
