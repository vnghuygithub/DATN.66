...
const Index = (props: SearchProps) => {
  ...
  const searchHandler = async () => {
    try {
      ...
      if (data) {
        if (updatedList) {
          updatedList = filterObjectsByNameAndCode(updatedList, data.name);
        }
        console.log('check seardch---', data.name);
    ...
    <>
      <Form
        ...
        <Row gutter={24}>
          ...
          <Col span={6}>
            <Form.Item label={'Nhập từ mã hoặc tên nhân viên'} name="name">
              <Input
                size="middle"
